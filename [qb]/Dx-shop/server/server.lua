...
Citizen.CreateThread(function()
    if Config.Core == "qb" then
        ...
        RegisterNetEvent('Dx-shop:makePayment', function(type, price, basket)
            local src = source
            ...
            if xPlayer.PlayerData.money[type] >= tonumber(price) then
                ...
                Core.Functions.Notify(src, "Your purchase was successful, bon appetit.", 'success')
            else
                Core.Functions.Notify(src, "You do not have sufficient funds.", 'error')
            end
        end)
